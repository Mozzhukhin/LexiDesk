...
def test_widget_uses_simple_reviews_and_four_choice_quizzes() -> None:
    ...
    assert "property bool adaptiveQuiz" not in qml
    ...
    assert "mixedDryStreak < 5 || !quizEligible" in qml
    assert "if (!adaptiveQuiz" not in qml
    ...
    assert "Incorrect — correct answer" not in qml
    assert "visible: answerChecked && !choiceMode" in qml
