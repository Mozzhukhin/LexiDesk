...
def execute_request(
    ...
    if command == "get":
        return word_payload(repository.get_word(int(request["word_id"])), repository)
    if command == "review":
        ...
        return word_payload(repository.next_word(word_id), repository)
    if command == "undo":
        restored = repository.undo_last_review()
        payload = word_payload(restored, repository)
